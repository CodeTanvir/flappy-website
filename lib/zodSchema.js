...
export const zSchema = z.object({
  ...
  .regex(/^01[0-9]{9}$/, "Invalid Bangladeshi phone number"),
    ...
    address: z.string().min(3, 'street is required'),
    zipcode: z.string().min(3,'zip is required').optional(),
    ordernote: z.string().min(3,'Ordernote is required').optional(),
   paymentMethod:z.enum(['cod','bkash']),
  
   bkashPhone:z.string().regex(/^01[0-9]{9}$/, "Invalid bKash number").optional(),
  trxId: z.string().min(8).optional(),
shipmentType: z.enum(["Hand Carry", "Agency"], {
  message: "Shipment type is required",
}),

date: z.string().min(1, "Date is required"),

costPerWeight: z.union([
  z.number().nonnegative("Cost per weight must be positive"),
  z.string()
...
totalWeight: z.union([
  z.number().nonnegative("Total weight must be positive"),
  z.string()
    .transform((val) => Number(val))
    .refine((val) => !isNaN(val) && val >= 0, {
      message: "Please enter a valid number",
