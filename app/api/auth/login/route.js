...
export async function POST(request){
    try{
        ...
        await sendMail('Email Verification request from Developer Tanvir'
            ,email,
            emailVerificationLink(
                `${process.env.NEXT_PUBLIC_BASE_URL}/auth/verify-email/${token}`));

            return response(false, 401, `Your email is not verified. 
      ...
      if(!otpEmailStatus.success){
        ...
        return response(true, 200, `Failed to Send Otp$${otp}`)
      }
